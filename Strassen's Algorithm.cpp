	...
	for (int i = 0; i < size_mat; ++i)
		for (int j = 0; j < size_mat; ++j)
			for (int k = 0; k < size_mat; ++k)
				mat_res[i * size_mat + j] += mat1[i * size_mat + k] * mat2[k * size_mat + j];
...
void split_4mat(float* mat, float* mat11, float* mat12, float* mat21, float* mat22, int size_mat)
{
	...
	for (int i = 0; i < new_size; ++i)
		for (int j = 0; j < new_size; ++j)
			mat11[i * new_size + j] = mat[i * size_mat + j];

	for (int i = 0; i < new_size; ++i)
		for (int j = new_size; j < size_mat; ++j)
			mat12[i * new_size + j - new_size] = mat[i * size_mat + j];

	for (int i = new_size; i < size_mat; ++i)
		for (int j = 0; j < new_size; ++j)
			mat21[(i - new_size) * new_size + j] = mat[i * size_mat + j];

	for (int i = new_size; i < size_mat; ++i)
		for (int j = new_size; j < size_mat; ++j)
			mat22[(i - new_size) * new_size + j - new_size] = mat[i * size_mat + j];
...
{
	int new_size = 2 * size_mat;
	float* mat = create_mat(new_size);

	for (int i = 0; i < size_mat; ++i)
		for (int j = 0; j < size_mat; ++j)
			mat[i * new_size + j] = mat11[i * size_mat + j];

	for (int i = 0; i < size_mat; ++i)
		for (int j = size_mat; j < new_size; ++j)
			mat[i * new_size + j] = mat12[i * size_mat + j - size_mat];

	for (int i = size_mat; i < new_size; ++i)
		for (int j = 0; j < size_mat; ++j)
			mat[i * new_size + j] = mat21[(i - size_mat) * size_mat + j];

	for (int i = size_mat; i < new_size; ++i)
		for (int j = size_mat; j < new_size; ++j)
			mat[i * new_size + j] = mat22[(i - size_mat) * size_mat + j - size_mat];
...
float* sum_mat(float* mat1, float* mat2, int size_mat)
{
	...
	for (int i = 0; i < size_mat; ++i)
		for (int j = 0; j < size_mat; ++j)
			mat[i * size_mat + j] = mat1[i * size_mat + j] + mat2[i * size_mat + j];
